...
/* Incluimos las variables para comunicarnos con el parser */
	...
	for (i=1; i<argc; i++)
	{
		...
		else
		{
			...
			if (input->isValidFile ())
			{
				...
				if (coin_3ds_read_file(input, root, 2, 25.f/180.f*M_PI, 
				...
				else
				{
					...
					/*
					//Comprobamos si es un fichero de netGen
					rewind (yyin);
					yyGeometry = import_netgen_mesh(yyin);
					if (yyGeometry)
					{
					//Cerramos el fichero
					fclose(yyin);
					...
					*/
					...
					//Comprobamos si es un fichero que podemos leer con parseOFF
					rewind (yyin);
					yyrestart(yyin);
					if (yyparse()==0)
					{
					...
					{
#ifdef USE_VOLEON
						//Comprobamos si es un fichero .vol
						rewind (yyin);
						char head[25];
						fread (head, 24, 1, yyin);
						if (head[0]==0x0b && head[1]==0x7e && 
							(head[23]==8 || head[23]==16))
						{

							//Añadimos un VolumeData+TransferFunction+VolumeRender
							SoVolumeData *voldata = new SoVolumeData();
							voldata->fileName = argv[i];
...
#endif
						{
							fprintf (stderr, "Unknow data format\n");
							exit (-1);
						}
					}
