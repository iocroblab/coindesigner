...
//Variable que indica si debemos mostrar informaci贸n al picar en
//la escena o no.
...
void MainWindow::CargarListaComponentes(ivPadre_t *listaComponentes)
{
	...
	while (comp->clase)
	{
		...
		else
		{
			...
			{
				//A帽adimos el nuevo nodo colgando del nodo padre
...
void MainWindow::cargar_fichero_locale(const char *fichero)
{
    ...
    //Construimos el nombre del fichero con el lenguaje y la extensi贸n
    QString Path;
...
//Esta funci贸n se encarga de mirar si tenemos programado alg煤n ayudante
//que nos permita editar f谩cilmente el campo. Los ayudantes son 
...
void MainWindow::fieldTable_clicked(int fila, int , int button)
{
   ...
   refreshGUI_but->setOn(false);
    ...
    {
       //Conversi贸n del SoField que estamos editando
...
{
    //Mostramos el nodo en la tabla de edici贸n de campos
