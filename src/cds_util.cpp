...
int ifs_patch_hole2(const SoPath *path, int f1, int f2)
{
   ...
   //A�ade una nueva faceta al final del indexedFaceSet
...
SoSeparator *ifs_recubrimiento(const SoPath *path, const float r)
{
	...
	//Contamos el n�mero de vertices y de facetas
    ...
    //Calculamos las normales por cara y v�rtice de la malla original
	...
	//Hacemos una pasada sobre la lista de vertices, a�adiendo un nuevo vertice 
	//desplazado seg�n la normal del vertice. 
	for (i=0; i < numVertex; i++)
	{
    ...
    for (int face=0; face<numFaces; face++)
    {
	   ...
	   //Comprobamos que es un triangulo v�lido
	   assert(v1>=0 && v2 >= 0 && v3 >= 0);

	   //Almacenamos informaci�n de vecindad, dando a cada semiarista un numero
	...
	//Hacemos una segunda pasada a las facetas, a�adiendo un triangulo por cada semiarista
...
//! Calcula la m�mima BBox no orientada con los ejes
...
//! Cambia la orientaci�n de todas las facetas de un IndexedFaceSet
...
int convex_hull (const SoMFVec3f &coords, SoMFVec3f &ch_coords, SoMFInt32 &ch_coordIndex)
{
    ...
    //Cambiamos el tama�o de ch_coordIndex
...
SoSeparator *import_tetgen (const char *nodeFilename)
{
	...
	{
		//A�adimos un indexedFaceSet para las facetas internas
		SoIndexedFaceSet *ifs = new SoIndexedFaceSet();
	...
	{
		//A�adimos un indexedFaceSet para las facetas externas
		SoIndexedFaceSet *ifs = new SoIndexedFaceSet();
...
char *cds_export_string (SoNode *node)
{
    ...
    //Leemos el buffer y reducimos su tama�o al minimo
...
bool cds_export_hppFile (SoNode *node, const char *className, const char *filename)
{
   ...
   while(line_end > -1)
   {
	   ...
	   if (line_end > 0)
	   {
                   ...
                   //Aislamos la linea y "escapamos" los caracteres problem�ticos
                   ...
                   //A�adimos los codigos necesarios para la compilacion al principio y al final
                   line.insert(0,"    \"");
                   line.append("\\n\",\n");
		   ...
		   //Actualizamos la posici�n del punto de inserccion
		   pos=strTemplate.find(code);
	   }
   ...
   //A�adimos codigo para cerrar la estructura y eliminamos el **scene_definition**
...
//Esta funcion est� pensada para mallas exportadas desde el vivid
