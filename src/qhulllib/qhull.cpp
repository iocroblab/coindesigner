...
void qh_qhull (void) {
  ...
  if (qh_setsize ((setT*)qhmem.tempstack) != 0) {
    gInterface->HullPrintf (qh ferr, "qhull internal error (qh_qhull): temporary sets not empty (%d)\n",
...
boolT qh_addpoint (pointT *furthest, facetT *facet, boolT checkdist) {
  ...
  if (!facet) {
    gInterface->HullPrintf (qh ferr, "qh_addpoint: NULL facet.  Need to call qh_findbestfacet first\n");
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
  if (checkdist) {
...
void qh_buildhull(void) {
  ...
  FORALLfacets {
    if (facet->visible || facet->newfacet) {
      gInterface->HullPrintf (qh ferr, "qhull internal error (qh_buildhull): visible or new facet f%d in facet list\n",
                   facet->id);    
  ...
  FORALLvertices {
    if (vertex->newlist) {
      gInterface->HullPrintf (qh ferr, "qhull internal error (qh_buildhull): new vertex f%d in vertex list\n",
                   vertex->id);
  ...
  if (qh num_outside && !furthest) {
    gInterface->HullPrintf (qh ferr, "qhull internal error (qh_buildhull): %d outside points were never processed.\n", qh num_outside);
    qh_errexit (qh_ERRqhull, NULL, NULL);
  }
...
At %02d:%02d:%02d & %2.5g CPU secs, qhull has created %d facets and merged %d.\n\
 ...
 outside points.  Next is point p%d (v%d), %2.2g above f%d.\n",
    ...
    cpu /= qh_SECticks;
    qh_distplane (furthest, facet, &dist);
    gInterface->HullPrintf (qh ferr, "qh_addpoint: add p%d (v%d) to hull of %d facets (%2.2g above f%d) and %d outside at %4.4g CPU secs.  Previous was p%d.\n",
  ...
  *goodvisible= *goodhorizon= 0;
  ...
  FORALLvisible_facets {
    if (visible->tricoplanar && !qh TRInormals) {
      gInterface->HullPrintf (qh ferr, "qh_findhorizon: does not work for tricoplanar facets.  Use option 'Q11'\n");
  ...
  if (!numhorizon) {
    ...
    gInterface->HullPrintf(qh ferr, "qhull precision error (qh_findhorizon): empty horizon\n\
...
pointT *qh_nextfurthest (facetT **visible) {
  ...
  while ((facet= qh facet_next) != qh facet_tail) {
    ...
    if (qh RANDOMoutside) {
      ...
      gInterface->HullPrintf (qh ferr, "qhull internal error (qh_nextfurthest): num_outside %d is too low\nby at least %d, or a random real %g >= 1.0\n",
...
void qh_partitioncoplanar (pointT *point, facetT *facet, realT *dist) {
  ...
  if (bestdist > qh max_outside) {
    ...
    if (bestdist > qh TRACEdist) {
      gInterface->HullPrintf (qh ferr, "qh_partitioncoplanar: ====== p%d from f%d increases max_outside to %2.2g of f%d last p%d\n",
  ...
  *numoutside= 0;
  FORALLvisible_facets {
    ...
    if (newfacet == qh facet_tail) {
      gInterface->HullPrintf (qh ferr, "qhull precision error (qh_partitionvisible): all new facets deleted as\n        degenerate facets. Can not continue.\n");
      qh_errexit (qh_ERRprec, NULL, NULL);
    }
...
Voronoi diagram by the convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    gInterface->HullPrintf(fp, "  Number of Voronoi regions%s: %d\n",
              qh ATinfinity ? " and at-infinity" : "", numvertices);
    if (numdel)
      gInterface->HullPrintf(fp, "  Total number of deleted points due to merging: %d\n", numdel); 
    if (numcoplanars - numdel > 0) 
      gInterface->HullPrintf(fp, "  Number of nearly incident points: %d\n", numcoplanars - numdel); 
    else if (size - numvertices - numdel > 0) 
      gInterface->HullPrintf(fp, "  Total number of nearly incident points: %d\n", size - numvertices - numdel); 
    gInterface->HullPrintf(fp, "  Number of%s Voronoi vertices: %d\n", 
              goodused ? " 'good'" : "", qh num_good);
    if (nonsimplicial) 
      gInterface->HullPrintf(fp, "  Number of%s non-simplicial Voronoi vertices: %d\n", 
...
Delaunay triangulation by the convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    gInterface->HullPrintf(fp, "  Number of input sites%s: %d\n", 
              qh ATinfinity ? " and at-infinity" : "", numvertices);
    if (numdel)
      gInterface->HullPrintf(fp, "  Total number of deleted points due to merging: %d\n", numdel); 
    if (numcoplanars - numdel > 0) 
      ...
      gInterface->HullPrintf(fp, "  Total number of nearly incident points: %d\n", size - numvertices - numdel); 
    gInterface->HullPrintf(fp, "  Number of%s Delaunay regions: %d\n", 
              goodused ? " 'good'" : "", qh num_good);
    if (nonsimplicial) 
      gInterface->HullPrintf(fp, "  Number of%s non-simplicial Delaunay regions: %d\n", 
              goodused ? " 'good'" : "", nonsimplicial);
  }else if (qh HALFspace) {
    gInterface->HullPrintf (fp, "\n\
Halfspace intersection by the convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    gInterface->HullPrintf(fp, "  Number of halfspaces: %d\n", size);
    gInterface->HullPrintf(fp, "  Number of non-redundant halfspaces: %d\n", numvertices);
    if (numcoplanars) {
      if (qh KEEPinside && qh KEEPcoplanar)
        ...
        s= "similar"; 
      gInterface->HullPrintf(fp, "  Number of %s halfspaces: %d\n", s, numcoplanars);
    } 
    gInterface->HullPrintf(fp, "  Number of intersection points: %d\n", qh num_facets - qh num_visible);
    if (goodused)
      gInterface->HullPrintf(fp, "  Number of 'good' intersection points: %d\n", qh num_good);
    if (nonsimplicial) 
      gInterface->HullPrintf(fp, "  Number of%s non-simplicial intersection points: %d\n", 
              goodused ? " 'good'" : "", nonsimplicial);
  }else {
    gInterface->HullPrintf (fp, "\n\
Convex hull of %d points in %d-d:\n\n", size, qh hull_dim);
    gInterface->HullPrintf(fp, "  Number of vertices: %d\n", numvertices);
    if (numcoplanars) {
      if (qh KEEPinside && qh KEEPcoplanar)
        ...
        s= "coplanar"; 
      gInterface->HullPrintf(fp, "  Number of %s points: %d\n", s, numcoplanars);
    } 
    gInterface->HullPrintf(fp, "  Number of facets: %d\n", qh num_facets - qh num_visible);
    if (goodused)
      gInterface->HullPrintf(fp, "  Number of 'good' facets: %d\n", qh num_good);
    if (nonsimplicial) 
      gInterface->HullPrintf(fp, "  Number of%s non-simplicial facets: %d\n", 
              goodused ? " 'good'" : "", nonsimplicial);
  }
  if (numtricoplanars)
      gInterface->HullPrintf(fp, "  Number of triangulated facets: %d\n", numtricoplanars);
  gInterface->HullPrintf(fp, "\nStatistics for: %s | %s", 
                      qh rbox_command, qh qhull_command);
  if (qh ROTATErandom != INT_MIN)
    gInterface->HullPrintf(fp, " QR%d\n\n", qh ROTATErandom);
  else
    gInterface->HullPrintf(fp, "\n\n");
  gInterface->HullPrintf(fp, "  Number of points processed: %d\n", zzval_(Zprocessed));
  gInterface->HullPrintf(fp, "  Number of hyperplanes created: %d\n", zzval_(Zsetplane));
  if (qh DELAUNAY)
    gInterface->HullPrintf(fp, "  Number of facets in hull: %d\n", qh num_facets - qh num_visible);
  gInterface->HullPrintf(fp, "  Number of distance tests for qhull: %d\n", zzval_(Zpartition)+
      zzval_(Zpartitionall)+zzval_(Znumvisibility)+zzval_(Zpartcoplanar));
#if 0  /* NOTE: must print before printstatistics() */
  {realT stddev, ave;
  gInterface->HullPrintf(fp, "  average new facet balance: %2.2g\n",
	  wval_(Wnewbalance)/zval_(Zprocessed));
  stddev= qh_stddev (zval_(Zprocessed), wval_(Wnewbalance), 
                                 wval_(Wnewbalance2), &ave);
  gInterface->HullPrintf(fp, "  new facet standard deviation: %2.2g\n", stddev);
  gInterface->HullPrintf(fp, "  average partition balance: %2.2g\n",
	  wval_(Wpbalance)/zval_(Zpbalance));
  stddev= qh_stddev (zval_(Zpbalance), wval_(Wpbalance), 
                                 wval_(Wpbalance2), &ave);
  gInterface->HullPrintf(fp, "  partition standard deviation: %2.2g\n", stddev);
  }
#endif
  if (nummerged) {
    gInterface->HullPrintf(fp,"  Number of distance tests for merging: %d\n",zzval_(Zbestdist)+
          zzval_(Zcentrumtests)+zzval_(Zdistconvex)+zzval_(Zdistcheck)+
          zzval_(Zdistzero));
    gInterface->HullPrintf(fp,"  Number of distance tests for checking: %d\n",zzval_(Zcheckpart));
    gInterface->HullPrintf(fp,"  Number of merged facets: %d\n", nummerged);
  }
  if (!qh RANDOMoutside && qh QHULLfinished) {
    cpu= qh hulltime;
    cpu /= qh_SECticks;
    wval_(Wcpu)= cpu;
    gInterface->HullPrintf (fp, "  CPU seconds to compute hull (after input): %2.4g\n", cpu);
  }
  if (qh RERUN) {
    if (!qh PREmerge && !qh MERGEexact)
      gInterface->HullPrintf(fp, "  Percentage of runs with precision errors: %4.1f\n",
	   zzval_(Zretry)*100.0/qh build_cnt);  /* careful of order */
  }else if (qh JOGGLEmax < REALmax/2) {
    if (zzval_(Zretry))
      gInterface->HullPrintf(fp, "  After %d retries, input joggled by: %2.2g\n",
         zzval_(Zretry), qh JOGGLEmax);
    else
      gInterface->HullPrintf(fp, "  Input joggled by: %2.2g\n", qh JOGGLEmax);
  }
  if (qh totarea != 0.0) 
    gInterface->HullPrintf(fp, "  %s facet area:   %2.8g\n", 
	    zzval_(Ztotmerge) ? "Approximate" : "Total", qh totarea);
  if (qh totvol != 0.0) 
    gInterface->HullPrintf(fp, "  %s volume:       %2.8g\n", 
	    zzval_(Ztotmerge) ? "Approximate" : "Total", qh totvol);
  if (qh MERGING) {
    qh_outerinner (NULL, &outerplane, &innerplane);
    if (outerplane > 2 * qh DISTround) {
      gInterface->HullPrintf(fp, "  Maximum distance of %spoint above facet: %2.2g", 
      ...
      if (ratio > 0.05 && 2* qh ONEmerge > qh MINoutside && qh JOGGLEmax > REALmax/2)
        gInterface->HullPrintf (fp, " (%.1fx)\n", ratio);
      else
        gInterface->HullPrintf (fp, "\n");
    }
    if (innerplane < -2 * qh DISTround) {
      gInterface->HullPrintf(fp, "  Maximum distance of %svertex below facet: %2.2g", 
